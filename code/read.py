...
if __name__ == '__main__':
    ...
    for year in years:
        ...
        for index, item in enumerate(uniques):
            ...
            life_remaining = data_to_plot[(data_to_plot[c4] > 0) & (~data_to_plot[c0].isin(revived))]
            logger.debug('life remaining shape: %s' % str(life_remaining.shape))
            life_remaining.plot(kind='scatter', x=c2, y=c4, ax=axes, c=color)
            no_life_remaining = data_to_plot[(data_to_plot[c4] == 0) & (~data_to_plot[c0].isin(revived))]
            logger.debug('no life remaining shape: %s' % str(no_life_remaining.shape))
            if len(no_life_remaining) > 0:
                no_life_remaining.plot(kind='scatter', x=c2, y=c4, ax=axes, c='r')
            new_life = data_to_plot[data_to_plot[c0].isin(revived)]
            if len(new_life) > 0:
                new_life.plot(kind='scatter', x=c2, y=c4, ax=axes, c=color_list[2])
